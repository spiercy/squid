...
static int peerWouldBePinged(const CachePeer *, ps_state *);
 ...
 */
bool
peerAllowedToUse(const CachePeer * p, ps_state * ps)
{
    assert(ps);
...
static int
peerWouldBePinged(const CachePeer * p, ps_state * ps)
{
    assert(ps);
    HttpRequest *request = ps->request;
...
int
peerHTTPOkay(const CachePeer * p, ps_state * ps)
{
...
CachePeer *
getRoundRobinParent(ps_state *ps)
{
    assert(ps);
    HttpRequest *request = ps->request;
...
CachePeer *
getWeightedRoundRobinParent(ps_state *ps)
{
    assert(ps);
    HttpRequest *request = ps->request;
...
lookup_t
peerDigestLookup(CachePeer * p, ps_state * ps)
{
...
CachePeer *
neighborsDigestSelect(ps_state *ps)
{
 ...
 */
static int
ignoreMulticastReply(CachePeer * p, ps_state * ps)
{
    if (p == NULL)
        return 0;
