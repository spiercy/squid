...
Security::PeerConnector::sslFinalized()
{
    ...
    if (!bypassCertChecks_) {
        ...
        // For now check for OCSP stapling only if the must-staple is
        // set in certificate.
        // For full OCSP support we need to check if the
        // SQUID_X509_V_ERR_OCSP_STAPLE_NOT_SUPPORTED error returned and
        // if yes, download OCSP status from Issuer server.
        if (Ssl::CertMustStapleIsSet(peerCert)) {
