...
bool Ssl::readCertFromMemory(Ssl::X509_Pointer & cert, char const * bufferToRead)
{
    Ssl::BIO_Pointer bio(BIO_new(BIO_s_mem()));
    BIO_puts(bio.get(), bufferToRead);
...
static void
printX509Signature(const Ssl::X509_Pointer &cert, std::string &out)
{
...
bool
Ssl::ReadX509Certificate(Ssl::BIO_Pointer &bio, Ssl::X509_Pointer & cert)
{
    assert(bio.get());
    if (X509 *certificate = PEM_read_bio_X509(bio.get(), NULL, NULL, NULL)) {
        cert.reset(certificate);
        return true;
    }
    return false;
}
...
bool
Ssl::WriteX509Certificate(Ssl::BIO_Pointer &bio, const Ssl::X509_Pointer & cert)
{
    if (!cert || !bio)
        return false;
    if (!PEM_write_bio_X509(bio.get(), cert.get()))
...
bool
Ssl::CertificatesCmp(const Ssl::X509_Pointer &cert1, const Ssl::X509_Pointer &cert2)
{
    if (!cert1 || ! cert2)
        return false;
