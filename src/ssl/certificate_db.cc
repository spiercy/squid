...
bool
Ssl::CertificateDb::find(std::string const &key,  const Ssl::X509_Pointer &expectedOrig, Ssl::X509_Pointer & cert, Ssl::EVP_PKEY_Pointer & pkey)
{
...
bool
Ssl::CertificateDb::addCertAndPrivateKey(std::string const & useKey, const Ssl::X509_Pointer & cert, const Ssl::EVP_PKEY_Pointer & pkey, const Ssl::X509_Pointer &orig)
{
    const Locker locker(dbLock, Here);
    load();
    if (!db || !cert || !pkey)
...
bool
Ssl::CertificateDb::pure_find(std::string const &key, const Ssl::X509_Pointer &expectedOrig, Ssl::X509_Pointer & cert, Ssl::EVP_PKEY_Pointer & pkey)
{
    if (!db)
        return false;
...
bool
Ssl::CertificateDb::WriteEntry(const std::string &filename, const Ssl::X509_Pointer & cert, const Ssl::EVP_PKEY_Pointer & pkey, const Ssl::X509_Pointer &orig)
{
    Ssl::BIO_Pointer bio;
    ...
    if (!Ssl::WritePrivateKey(bio, pkey))
        return false;
    if (orig.get() && !Ssl::WriteX509Certificate(bio, orig))
        return false;
    return true;
}

bool
Ssl::CertificateDb::ReadEntry(std::string filename, Ssl::X509_Pointer & cert, Ssl::EVP_PKEY_Pointer & pkey, Ssl::X509_Pointer &orig)
{
    Ssl::BIO_Pointer bio;
