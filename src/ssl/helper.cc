...
static void
sslCrtvdHandleReplyWrapper(void *data, char *reply)
{
    ...
    if (replyMsg.parse(reply, strlen(reply)) != Ssl::CrtdMessage::OK ||
        ...
        validationResponse->resultCode = "BH";
    }
    else
        validationResponse->resultCode = replyMsg.getCode();

    crtdvdData->callback(crtdvdData->data, *validationResponse);

    if (Ssl::CertValidationHelper::HelperCache &&
        (validationResponse->resultCode == "OK" || validationResponse->resultCode == "ERR")) {
...
void Ssl::CertValidationHelper::sslSubmit(Ssl::CertValidationRequest const &request, Ssl::CertValidationHelper::CVHCB * callback, void * data)
{
    ...
    if (ssl_crt_validator->stats.queue_size >= (int)(ssl_crt_validator->childs.n_running * 2)) {
        ...
        resp.resultCode = "error 45 Temporary network problem, please retry later";
        callback(data, resp);
        return;
    }
    first_warn = 0;

    Ssl::CertValidationMsg message;
